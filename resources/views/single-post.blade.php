...
<body>
    ...
    <div class="container w-50">
        <div class="mt-4 mb-3" style="text-align:center">
            <span class="h2"><strong><a href="{{ route('post.getallpost') }}">게시판</a></strong></span>
        </div>
        <div class="mb-2 mt-5">
            <p>
            <h5 style="text-align:right">작성자 : {{ $post->name }} </h5>
            </p>
        </div>
        ...
        @if (isset($post->img_path))
            <div class="mt-5">
                <img src="{{ asset($post->img_path) }}" style="max-width: 500px; max-height:500px; object-fit:cover">
            </div>
        @endif
        <div class="mt-5">
        ...
        @auth
            <div class="w-4/5 mx-auto mt-6 text-right">
                <form method="post" action="{{ route('comment.add') }}">
                    ...
                    <div style="margin-top:55px">
                        ...
                        <textarea name="commentStory" cols="1" rows="5" class="form-control" style="resize: none"
            ...
            @else
                <div style="margin-top:55px">
                    <span class="h5">댓글 작성</span>
                    <textarea name="commentStory" cols="1" rows="5" class="form-control" style="resize: none"
                        placeholder="로그인 시에 댓글 작성 가능합니다"></textarea>
                </div>
                <div style="margin-top:10px">
            ...
            @if ($comment->count() > 0)
                @foreach ($comment as $comments)
                    @if ($comments->topcomment_id === null)
                                ...
                                <div class="card-body">
                                    ...
                                    @if ($comments->userID == auth()->id())
                                        <button type="button" class="btn btn-secondary" data-bs-toggle="modal"
                                            data-bs-target="#staticBackdrop{{ $comments->id }}">
                                            수정
                                        </button>
                                        <!-- 삭제 버튼 -->
                                        ...
                                        @if ($reply->topcomment_id == $comments->id)
                                            <div class="card" style="width: 40rem;">
                                                <ul class="list-group list-group-flush">
                                                    <li class="list-group-item">ㄴ
                                                        {{ $reply->userName . ' : ' . $reply->commentStory }}</li>
                                                </ul>
                                            </div>
                                        @endif
                                    @endforeach
                                        ...
                                        <div class="comment-container">
                                            <div class="dropdown">
                                                <button class="btn btn-primary dropdown-toggle" type="button"
                                                    id="replyDropdown{{ $comments->id }}" data-bs-toggle="dropdown"
                                                    aria-expanded="false">
                                                    답글 작성
                                                </button>
                                                <form action="{{ route('comment.addreply') }}" method="POST">
                                                    @csrf
                                                    <input type="hidden" name="topcomment_id"
                                                        value="{{ $comments->topcomment_id ?? $comments->id }}">
                                                    <!-- topcomment_id를 hidden input으로 전달 -->
                                                    <input type="hidden" name="parent_id" value="{{ $post->id }}">
                                                    <!-- 답글이 적힌 글의 id를 받아옴 -->
                                                    <ul class="dropdown-menu"
                                                        aria-labelledby="replyDropdown{{ $comments->id }}">
                                                        <li>
                                                            <textarea class="form-control reply-textarea" name="commentStory" rows="3" style="resize: none"></textarea>
                                                        </li>
                                                        <li>
                                                            <button type="submit"
                                                                class="btn btn-success mt-2 submit-reply-btn">작성</button>
                                                        </li>
                                                    </ul>
                                                </form>
                                            </div>
                        ...
                        <!-- 수정 모달 -->
                        <div class="modal fade" id="staticBackdrop{{ $comments->id }}" data-bs-backdrop="static"
